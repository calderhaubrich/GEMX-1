...
!         vxdum = eyp+vpar/b*delbxp
         ...
         z3(m)=modulo(z3(m)+8.*lz,lz)
         x3(m)=modulo(x3(m)+8.*lx,lx)         
...
!SP            w3(m) = 0.
!SP         end if
!SP         z3(m)=modulo(z3(m)+8.*lz,lz)
!SP         x3(m)=modulo(x3(m)+8.*lx,lx)         
...
!SP      !open(935, file='plot1',status='unknown',position='append')
!SP      write(935,*) timestep,m, (x2(m))*xu+Rgrid(0),(z2(m))*xu+Zgrid(0)
!SP     close(935)
...
!     load a slab of ions...
         ...
         dumy=zdim*ran2(iseed) !revers(MyId*cnt+j,3) !ran2(iseed)
...
!     load a slab of ions...
         ...
         dumx=xdim*(0.02+0.96/float(mm(1))*j)
         dumy=lz*0.5
         dumz=pi2*ran2(iseed) !revers(MyId*cnt+j,5) !ran2(iseed)

         x2(j)=min(dumx,xdim-1.d-8)
         z2(j)=min(dumy,zdim-1.d-8)
         zeta2(j)=dumz
